...
func TestParseAgentTypeAndNameFlag(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "agent type with name flag",
			args:          []string{"codex", "-n", "myjob"},
			wantAgentType: "codex",
			wantName:      "myjob",
		},
		{
		...
		{
			name:          "--name= syntax",
			args:          []string{"gemini", "--name=myjob"},
			wantAgentType: "gemini",
			wantName:      "myjob",
		},
		{
			name:          "agent type before double-dash separator",
			args:          []string{"opencode", "--", "--some-flag"},
		...
		{
			name:          "name flag then double-dash",
			args:          []string{"claude", "-n", "foo", "--", "--verbose"},
		...
		},
		{
			...
			args:          []string{"claude", "--unknown-flag"},
			wantAgentType: "claude",
